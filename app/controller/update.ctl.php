<?php
    ...
    class zajapp_update extends zajController {
        ...
        function __load($request, $optional_params = []) {
            // is update disabled?
            if (!$this->ofw->ofwconf['update_enabled']) {
                return exit("Update disabled.");
            }

            // check for recommended updates
            if (defined('OFW_RECOMMENDED_HTACCESS_VERSION') && OFW_RECOMMENDED_HTACCESS_VERSION > $this->ofw->htver) {
                $this->ofw->variable->htver_upgrade = OFW_RECOMMENDED_HTACCESS_VERSION;
            }
            if (defined('OFW_RECOMMENDED_CONFIG_VERSION') && OFW_RECOMMENDED_CONFIG_VERSION > $this->ofw->ofwconf['config_file_version']) {
                $this->ofw->variable->conf_upgrade = OFW_RECOMMENDED_CONFIG_VERSION;
            }
            ...
            if (!$this->ofw->debug_mode) {
                ...
                if (!$this->ofw->ofwconf['update_user'] || !$this->ofw->ofwconf['update_password']) {
                    return $this->install();
                }
                // realm
                if (!empty($this->ofw->ofwconf['update_realm'])) {
                    $realm = $this->ofw->ofwconf['update_realm'];
                } else {
                    $realm = "Outlast Framework Update";
                }
                ...
                if (!empty($_REQUEST['update_user'])) {
                    ...
                    return $this->ofw->security->protect($this->ofw->ofwconf['update_user'],
                        $this->ofw->ofwconf['update_password'], $realm, $denied_message);
                }
            }
        ...
        function deploy() {
            ...
            // Run sql update dry run and throw 500 error if changes needed
            if ($this->ofw->ofwconf['mysql_enabled']) {
                $this->ofw->variable->dbresults = (object)$this->ofw->model->update(true);
                if ($this->ofw->variable->dbresults->num_of_changes > 0) {
                    header('HTTP/1.1 500 Internal Server Error');
                }
            }

            // Run unit tests if no mysql changes waiting
			if ($this->ofw->variable->dbresults->num_of_changes == 0) {
                $this->test(false);
            }
            // Get test results
            $this->ofw->variable->testresults = $this->ofw->template->block("update/update-test.html", "testresults",
                false, false, true);
        ...
        function database_run() {
            ...
            if ($db_update_result['num_of_changes'] >= 0) {
                print $db_update_result['log'];
            } else {
        ...
        function test($show_result = true) {
            ...
            // Display!
            if ($show_result) {
                return $this->ofw->template->show("update/update-test.html");
            } else {
                return $result;
            }
        ...
        function install() {
            ...
            foreach (array_reverse($this->ofw->ofwconf['plugin_apps']) as $plugin) {
                ...
                if (file_exists($this->ofw->basepath.'plugins/'.$plugin.'/controller/'.$plugin.'.ctl.php') || file_exists($this->ofw->basepath.'plugins/'.$plugin.'/controller/'.$plugin.'/default.ctl.php')) {
                    ...
                    $result = $this->ofw->reroute($plugin.'/__install/', [], false, false);
                    // __install should return a string if it fails, otherwise it is considered to pass
                    $plugin_text = "";
                    if (is_string($result) && $result == ZAJ_INSTALL_DONTCHECK) {
                        return true;
                    } else if (is_string($result)) {
                        $plugin_text .= "<li>Checking plugin $plugin. <span class='label label-important'>Failed</span><pre class='well' style='font-family: monospace; padding: 10px; overflow: auto; background-color: #f5f5f5; margin-top: 10px;'>$result</pre></li>";
                        $ready_to_activate = false;
                    } else {
                        $plugin_text .= "<li>Checking plugin $plugin... <span class='label label-success'>Done</span></li>";
                    }
                }
            }
            // Check status for each step
            // 1. Check writable
            if (!is_writable($this->ofw->basepath."cache/") || !is_writable($this->ofw->basepath."data/")) {
                $status_write = $todo;
                $ready_to_dbupdate = false;
                $ready_to_activate = false;
            } else {
                $status_write = $done;
            }
            ...
            if (!$this->ofw->ofwconf['mysql_enabled']) {
                ...
                if ($this->ofw->db->connect($this->ofw->ofwconf['mysql_server'], $this->ofw->ofwconf['mysql_user'],
                    $this->ofw->ofwconf['mysql_password'], $this->ofw->ofwconf['mysql_db'], false)) {
                    $status_db = $done;
                } else {
                    $status_db = $todo;
                    $ready_to_dbupdate = false;
            ...
            if (empty($this->ofw->ofwconf['update_user']) || empty($this->ofw->ofwconf['update_password'])) {
                if ($this->ofw->debug_mode) {
                    $status_updatepass = $optional;
                } else {
                    $status_updatepass = $todo;
                    $ready_to_activate = false;
                }
            } else {
                $status_updatepass = $done;
            }
            // 4. Check database update (photo table should always exist)
            if (!$this->ofw->ofwconf['mysql_enabled']) {
                $status_dbupdate = $na;
            } else if ($status_db == $todo) {
                $status_dbupdate = $todo;
                $ready_to_activate = false;
            } else {
                ...
                if ($result->c <= 0) {
                    ...
                    $status_dbupdate = $done;
                }
            }
            // 5. Check activation
            if (!is_object($this->ofw->mozajik) || !MozajikVersion::check()) {
                $status_activate = $todo;
            } else {
                $status_activate = $done;
            }
            ?>
			<head>
				<meta charset="utf-8">
				...
				<script language="JavaScript"
						src="<?php echo $this->ofw->baseurl; ?>system/js/jquery/jquery-1.8.0.min.js"
						type="text/javascript"></script>
				<script language="JavaScript" src="<?php echo $this->ofw->baseurl; ?>system/js/mozajik-base-jquery.js"
						type="text/javascript"></script>
			...
			<div class="container">
				<div class="row">
					<div class="span12">
						...
						<h3>Welcome to the Outlast Framework installation for
							version <?php echo MozajikVersion::$major; ?>
							.<?php echo MozajikVersion::$minor; ?> <?php if (MozajikVersion::$beta) {
                                echo "beta";
                            } ?></h3>
                        <?php if ($this->ofw->debug_mode) { ?><h5><span style="color: red;">Attention!</span> This
							installation will be running in <strong>debug mode</strong>. This is not recommended for
							production sites!</h5><?php } ?>
						<hr/>
						<ul>
                            <?php if (empty($plugin_text)) {
                                echo "<li>Checking plugins... No plugins activated.</li>";
                            } else echo $plugin_text ?>
						</ul>
						<hr/>
						<ol>
							<li>Make /cache and /data folders writable by webserver. - <?php echo $status_write; ?></li>
							<li>Create read/write permissions for database (if mysql is enabled).
								- <?php echo $status_db; ?></li>
							<li>Create an update user and password in /site/index.php (required if in production mode).
								- <?php echo $status_updatepass; ?></li>
                            ...
                            <?php if ($ready_to_activate && $status_activate == $done) { ?><br/>
								<div class="alert alert-success center">Your installation is currently <span
										style="color:green;">activated</span>. Go to the <a
										href="<?php echo $this->ofw->baseurl; ?>">home page</a>.</div><?php } ?>
						</ol>
						<hr/>
					</div>
				</div>
				<div class="row">
					...
					<div class="span4 center">
						<input class="btn btn-primary" type="button"
							   onclick="ofw.open('<?php echo $this->ofw->baseurl; ?>update/database/<?php echo $this->update_user_query; ?>', 1000, 500);"
                               <?php if (!$ready_to_dbupdate){ ?>disabled="disabled"<?php } ?>
							   value="Update the database">
					</div>
					<div class="span4 center">
						<input class="btn btn-success" type="submit"
							   onclick="window.location = '<?php echo $this->ofw->baseurl; ?>update/install/go/<?php echo $this->update_user_query; ?>';"
                               <?php if (!$ready_to_activate || $status_activate == $done){ ?>disabled="disabled"<?php } ?>
							   value="Activate this installation">
					</div>
				</div>
				<div class="row">
					<div class="span12 center">
						<br/>
						<a href="<?php echo $this->ofw->baseurl; ?>update/<?php echo $this->update_user_query; ?>">Back
							to the update page</a>
					</div>
				</div>
			</div>
			</body>
        ...
        function install_go($redirect_me = true) {
            ...
            if ($redirect_me) {
                return $this->ofw->redirect("update/install/".$this->update_user_query);
            } else {
                return true;
            }
        }
        ...
        function progress() {
			...
			<head>
				...
				<script language="JavaScript"
						src="<?php echo $this->ofw->baseurl; ?>system/js/mootools/mootools-core-1.3.js"
						type="text/javascript"></script>
				<script language="JavaScript"
						src="<?php echo $this->ofw->baseurl; ?>system/js/mootools/mootools-more-1.3.js"
						type="text/javascript"></script>
				<script language="JavaScript" src="<?php echo $this->ofw->baseurl; ?>system/js/mozajik-base-1.3.js"
						type="text/javascript"></script>
