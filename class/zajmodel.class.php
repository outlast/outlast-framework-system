...
abstract class zajModel implements JsonSerializable {
	...
	public static function get_cache($id){
		...
		// Get current class and sanitize id
		/* @var zajModel $class_name */
		$class_name = get_called_class();
		...
		// Try opening file and unserializing
		/* @var zajModel $new_object */
		$item_cached = false;
		if(!file_exists($filename)){
			$new_object = new $class_name($id, $class_name);
			if($new_object::$in_database) $new_object->__get('name'); // get my name (this will also grab the db)
		}
		else{
		...
		// If this is an extended object, then
		/* @var zajModelExtender $ext */
        $ext = $class_name::extension();
        if($ext) $new_object = $ext::create($new_object->id, $new_object);
