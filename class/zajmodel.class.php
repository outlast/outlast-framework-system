...
abstract class zajModel implements JsonSerializable {
	 ...
	 * @param string|zajModel|zajModelExtender|null $id OPTIONAL. The id of the object. Leave empty if you want to fetch multiple objects. You can also pass an existing zajModel object in which case it will simply pass through the function without change - this is useful so you can easily support both id's and existing objects in a function.
	 * @return zajFetcher|zajModel|zajModelExtender|null Returns a zajFetcher object (for multiple objects) or a zajModel object (for single objects) or null if failed to fetch.
	 */
	public static function fetch(string|zajModel|zajModelExtender|null $id = null) : zajFetcher|zajModel|zajModelExtender|null {
		// Get my class_name
			/* @var string|zajModel $class_name */
			$class_name = get_called_class();
        // Arguments passed?
	 ...
	 * @param ?string $id Identifiers are automatically generated by default, but you can force one here. Once created, id's cannot be changed.
	 ...
	 * @return zajModel|zajModelExtender Returns an object whose fields are set to the same data values.
	 */
	public function duplicate(string|int|null $id = null) : self|zajModelExtender {
		// Get my class name
			/* @var zajModel $class_name */
	...
	public function to_array(bool $localized = true) : array {
		...
		// Load up data if not yet loaded
			if($localized){
				if(!isset($this->translations)) $this->translations = new zajModelLocalizer($this);
	...
	public function __get($name){
		...
		switch($name){
			...
			case "name":
                if(!$this::$in_database || isset($this->name)) return $this->name ?? "";
	 ...
	 * @return zajModel|zajModelExtender|null Returns the object or null if failed.
	 * @ignore
	 * @todo Disable get_cache from being called outside. Events should be used instead of overriding...
	 */
	public static function get_cache(string $id) : zajModel|zajModelExtender|null {
		// get current class
		$class_name = get_called_class();
