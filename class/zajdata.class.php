<?php
    ...
    class zajData {
        ...
        public function reload() {
            ...
            $this->reset();
            // load from db
            $this->data = $this->db->select("SELECT * FROM `{$this->zajobject->table_name}` WHERE `{$this->zajobject->id_column}`='".addslashes($this->zajobject->id)."' LIMIT 1",
                true);
        ...
        public function save() {
            ...
            if (!$this->exists && !$this->init()) {
                return $this->zajobject->zajlib->warning('save failed! could not initialize object in database!');
            }
        ...
        private function init() {
            // init default fields
            if (empty($this->data['time_create'])) {
            ...
            if (!$result) {
                $this->zajobject->zajlib->warning("SQL SAVE ERROR: ".$this->db->get_error()." / ".$this->db->get_last_query()." <a href='{$this->zajobject->zajlib->baseurl}update/database/'>Update needed?</a>");
            }
            // set that i exist
            $this->exists = true;
        ...
        public function __get($name) {
            ...
            if (!$this->zajobject->model->$name) {
                return $this->zajobject->zajlib->warning("Cannot get value of '$name'. field '$name' does not exist in model '{$this->zajobject->class_name}'!");
            }
        ...
        public function __set($name, $value) {
            // check for error
            if (!$this->zajobject->model->$name) {
                return $this->zajobject->zajlib->warning("cannot set value of '$name'. field '$name' does not exist in model '{$this->zajobject->class_name}'!");
            }
