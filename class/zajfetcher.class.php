<?php
    ...
    class zajFetcher implements Iterator, Countable, JsonSerializable {
        ...
        private $select_what = [];                        // what to select from db. the index is the 'as' value.
        private $select_from = [];                        // the tables to select from
        ...
        public function paginate($perpage = 10, $page = false) {
            ...
            if (!$perpage) {
                ...
                if (!empty($_GET['zajpagination']) && $page === false) {
         ...
         * @return zajFetcher This method can be chained.
         **/
        public function exclude($field, $value, $operator = 'NOT LIKE', $type = 'AND') {
         ...
         * @return zajFetcher This method can be chained.
         **/
        public function exc($field, $value, $operator = 'NOT LIKE', $type = 'AND') {
            return $this->filter($field, $value, $operator, $type);
        }
         ...
         * @return zajFetcher This method can be chained.
         **/
        public function inc($field, $value, $operator = 'LIKE', $type = 'OR') {
            return $this->filter($field, $value, $operator, $type);
        }
         ...
         * @return zajFetcher|bool This method can be chained.
         **/
        public function before($value, $field = 'time_create', $type = 'AND') {
            ...
            if (zajModel::is_instance_of_me($value)) {
                ...
                if ($value->class_name != $this->class_name) {
                    return zajLib::me()->error("Fetcher's before() method only supports using the same model. You tried using '$value->class_name' while this fetcher is a '$this->class_name'.");
                }
        ...
        public function after($value, $field = 'time_create', $type = 'AND') {
            ...
            if (zajModel::is_instance_of_me($value)) {
                // check error
                if ($value->class_name != $this->class_name) {
                    return zajLib::me()->error("Fetcher's after() method only supports using the same model. You tried using '$value->class_name' while this fetcher is a '$this->class_name'.");
                }
                // check my default sort order
                /** @var zajModel $class_name */
                $class_name = $value->class_name;
        ...
        public function limit($startat, $limitto = false) {
            ...
            if ($startat === false || !is_numeric($startat) || ($limitto !== false && !is_numeric($limitto))) {
                $this->limit = "";
            } // set limit to value
            else {
...
EOF;
        ...
        private function get_connection_table_details() {
            ...
            if ($this->connection_type != 'manytomany') {
                return zajLib::me()->error("Tried to get connection table for something other than manytomany.");
            }
