<?php
    ...
    class zajlib_template extends zajLibExtension {
        ...
        private function prepare($source_path, $force_recompile = false, $destination_path = false) {
            ...
            if (!is_object($this->ofw->variable->ofw) || !is_a($this->ofw->variable->ofw,
                    'ofw_template_zajvariables')) {
                $this->ofw->variable->ofw = new ofw_template_zajvariables($this->ofw);
        ...
        public function get_variables() {
            return new ofw_template_zajvariables($this->ofw);
        }
     ...
     * @property zajLib $ofw
     **/
    class ofw_template_zajvariables {
        private $ofw;    // The local copy of ofw variable
         ...
         **/
        public function __construct($ofw) {
            // First get my ofw
            $this->ofw = $ofw;
            // Important variables
        ...
        public function __get($name) {
            switch ($name) {
                ...
                case 'js':
                    ...
                    if ($this->ofw->ofwconf->trackevents_analytics === false) {
                        $trackevents_analytics = 'false';
                    } else {
                        $trackevents_analytics = 'true';
                    }
                    if ($this->ofw->ofwconf->trackevents_local === true) {
