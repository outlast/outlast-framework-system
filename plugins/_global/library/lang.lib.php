    ...
    class zajlib_lang extends zajlib_config {
        ...
        public function set(string|bool $new_locale = false) : string {
            ...
            if (!empty($new_locale) && $this->is_valid_locale($new_locale)) {
                ...
                if (!empty($new_locale) && $this->ofw->config->variable->lang_warn_locale_set_not_found) {
                    $this->ofw->warning("Requested locale $new_locale not found, using default locale (".$this->default_locale.") instead.");
                }
        ...
        public function auto() : string {
            ...
            if (!empty($_GET['locale'])) {
                ...
                if (strlen($_GET['lang']) == 2) {
                    $this->set_by_code($_GET['lang']);
                } else {
                    $this->set($_GET['lang']);
                }
            } // If a cookie is set, use that
            else if (!empty($cookie_value) && isset($cookie_value)) {
                $this->set($cookie_value);
            } // If an Apache variable is set, use that
            else if (!empty($_SERVER['OFW_LOCALE'])) {
                $this->set($_SERVER['OFW_LOCALE']);
            } // If the subdomain is two letters, it will consider it a language code
            else if (strlen($this->ofw->subdomain) == 2) {
                $this->set_by_code($this->ofw->subdomain);
            } // If the tld is two letters, it will consider it a language code
            else if (strlen($this->ofw->tld) == 2) {
                $this->set_by_code($this->ofw->tld);
            ...
            if (empty($cookie_value) || $current != $cookie_value) {
                if (empty($_GET['disable_locale_cookie']) && $this->ofw->output_started === false) {
                    $this->ofw->cookie->set('ofw_locale', $current);
                }
            }
        ...
        function template(
            ...
            if (file_exists($this->ofw->basepath.'app/view/'.$base_source_path.'.'.$this->default_locale.'.html')) {
                return $this->ofw->template->show($base_source_path.'.'.$this->default_locale.'.html', $force_recompile,
                    $return_contents, $custom_compile_destination);
            }

            // All failed, so finally, just include me
            return $this->ofw->template->show($source_path, $force_recompile, $return_contents,
                $custom_compile_destination);
        }
        ...
        function block($source_path, $block_name, $force_recompile = false, $return_contents = false) {
            ...
            if (file_exists($this->ofw->basepath.'app/view/'.$base_source_path.'.'.$this->current_locale.'.html')) {
                return $this->ofw->template->block($base_source_path.'.'.$this->current_locale.'.html', $block_name,
                    $force_recompile, $return_contents);
            }
            // Next for default locale
            if (file_exists($this->ofw->basepath.'app/view/'.$base_source_path.'.'.$this->default_locale.'.html')) {
                return $this->ofw->template->block($base_source_path.'.'.$this->default_locale.'.html', $block_name,
                    $force_recompile, $return_contents);
            }
            ...
            if (!$this->is_default_locale()) {
                ...
                if (!$current_result && !$this->ofw->test->is_running() && property_exists($this->ofw->config->variable, 'lang_show_warning_when_cant_load_in_current_locale') && $warn_on_error) {
                    if ($section === false) {
                        ...
                        $this->ofw->warning("The section $section in language file $current_source_path was not found, trying default locale.");
                    }
                }
            }
