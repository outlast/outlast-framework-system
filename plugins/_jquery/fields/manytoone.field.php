	...
	public function save($data, &$object){
		...
		if(is_object($data)){
			// check to see if zajModel
				if(!is_a($data, 'zajModel') && !is_a($data, 'zajModelExtender')) $this->zajlib->error('Problem found on '.$object->table_name.'.'.$this->name.': Manytoone connections only accept single model objects!');
		...
		// assemble code
			...
			elseif(is_array($value)){
				...
				// Possible values: object, string, boolean false
					if(is_object($value) && (is_a($value, 'zajModel') || is_a($value, 'zajModelExtender'))) $value = $value->id;
